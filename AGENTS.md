...
- Hard limit: one code-writing operation, patch, or commit may add or modify at most 100 lines of code in total across all files. Count source, test, style, and executable configuration lines. If a necessary step would exceed 100 lines, stop and obtain the user's explicit manual approval before making that edit; approval cannot be inferred or requested after the fact.
- Do not evade the 100-line limit with generated rewrites or meaningless fragments. Split work at behaviorally sensible boundaries and verify each boundary.
